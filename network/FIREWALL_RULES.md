...
| 4 | 🚫 BLOCK | any | opt5 net | `__qfeeds_malware_ip` | any | **Ingress QFeeds** — block known malicious IPs (threat feed) |
...
### Threat Feed / qfeeds Aliases

| Alias Name | Type | Content | Description |
|------------|------|---------|-------------|
| **__qfeeds_malware_ip** | alias / URL table (auto-generated by qfeeds plugin) | qfeeds malware IP feed | Active threat-feed alias used in `Ingress QFeeds` BLOCK rules on WAN, LAN, VLAN 20 (opt4), and VLAN 30 (opt5) |
...
5. **Layer 5 - Threat Intelligence**: qfeeds malware IP feed blocking on WAN, LAN, VLAN 20, VLAN 30
...
- ✅ Known malicious IPs blocked by `__qfeeds_malware_ip` (Ingress QFeeds rule)
...
**Logged Events**:
- ❌ BLOCK rules: sequence 1 (opt4 — Internal_Networks + Ingress QFeeds), sequence 4 (opt5 — DMZ + Ingress QFeeds), sequence 4-5 (opt6)
